...
class MyScene extends THREE.Scene {
  ...
  getPickableObjects(){
    ...
    const pickableObjects = [puertaSuperior,puertaInferior];
  ...
  pick(event){
    var mouse = new THREE.Vector2((event.clientX/window.innerWidth)*2-1, 1-2*(event.clientY/window.innerHeight));
    ...
    if(pickedObjects.length > 0){
      ...
      if(selectedObject.userData.isObject3D)
        if(selectedObject.name=="puertaSuperior"){
          selectedObject.userData.recibeClick(true);
        }else
          selectedObject.userData.recibeClick(false);
